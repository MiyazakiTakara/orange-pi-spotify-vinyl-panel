...
if ! id "$APP_USER" >/dev/null 2>&1; then
  echo "User $APP_USER does not exist. Create it or run: APP_USER=patryk sudo ./install.sh" >&2
  exit 1
fi
...
cp config/spotify-panel.env.example /etc/default/spotify-panel
...
# Let systemd find the local package without a global pip install.
...
PYTHONPATH="$PYTHON_SITE" python3 -m py_compile "$PYTHON_SITE/vinyl_panel/server.py"
